...
# dependencies = []
# ///
"""Install harness-eval dependencies into an isolated venv.
...
def main():
    ...
    if not venv_python or not _all_importable(venv_python, DEPS):
        print(
            "harness-eval: deps installed but some imports failed",
            file=sys.stderr,
        )
        sys.exit(1)
...
def _ensure_venv(venv_dir):
    ...
    uv = shutil.which("uv")
    if uv:
        print(f"harness-eval: creating venv with uv: {venv_dir}")
        subprocess.run(
        ...
        print(f"harness-eval: creating venv: {venv_dir}")
        subprocess.run(
...
def _install_deps(venv_dir, specs):
    ...
    venv_python = _find_venv_python(venv_dir)
    if not venv_python:
        print("harness-eval: no python found in venv", file=sys.stderr)
        sys.exit(1)

    uv = shutil.which("uv")
    venv_pip = venv_dir / "bin" / "pip"

    print(f"harness-eval: installing {', '.join(specs)}")
    ...
    if result.returncode != 0:
        print(f"harness-eval: install failed: {result.stderr}", file=sys.stderr)
        sys.exit(1)
